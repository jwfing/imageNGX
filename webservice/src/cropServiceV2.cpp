...
LoggerPtr CropServiceV2::_logger = Logger::getLogger("cn.leancloud.imageService.cropV2");
const static string IMAGE_TYPE = "image/jpeg";
const static string JSON_TYPE = "application/json";

CropServiceV2::CropServiceV2(Thumbnailer* thumbnailer,
...
CropServiceV2::~CropServiceV2() {
    _thumbnailer = NULL;
}

void CropServiceV2::parseQueryString(const string& queryString,
                                     int& w,
                                     int& h,
                                     int& hostStyle,
                                     int& extraInfo,
...
bool CropServiceV2::shouldOriginalImage(int w, int h, ImageOperator* opers) {
    if (w == -1 && h == -1
        && opers->getFmt().empty()
...
int CropServiceV2::generateFileName(char* buffer,
                                    int bufferLen,
				    const string& bucket,
                                    const string& key,
                                    int width,
...
string CropServiceV2::getOriginalFileUrl(const string& base,
                                         const string& bucket,
...
void CropServiceV2::sendImage(HTTPResponseWriterPtr writer,
                              Image* result,
                              const string& fileName) {
    int dataLen = 0;
...
int CropServiceV2::doThumbnail(const char* src, size_t lenOfSrc,
...
void CropServiceV2::handleNotFound(HTTPResponseWriterPtr writer,
                                   const string& bucket,
                                   int hostStyle,
                                   int targetWidth,
                                   int targetHeight) {
    string url = "";
    url = getOriginalFileUrl("file", bucket, CONF_NOT_FOUND_IMAGE, hostStyle);
    redirectResponse(writer, url);
}

void CropServiceV2::operator()(const HTTPRequestPtr& request,
    ...
    int res = generateFileName(fileName, 1024, bucket, key, w, h,
                               opers,
                               isOriginal);
    if (res < 0) {
        if (opers != NULL) {
    ...
    if (isOriginal) {
        ...
        string url = getOriginalFileUrl("file", bucket, fileName, hostStyle);
        redirectResponse(writer, url);
